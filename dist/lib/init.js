...
export async function initTDD(options = {}) {
    const { workflows = true, docs = true, scripts = true, force = false } = options;
    ...
    // Optional scripts
    if (scripts) {
        filesToCopy.push({ src: 'scripts/tdd-feature.js', dest: 'scripts/tdd-feature.js' }, { src: 'scripts/create-tdd-component.js', dest: 'scripts/create-tdd-component.js' });
    }
    // Optional files
    if (workflows) {
        filesToCopy.push({ src: 'github/workflows/auto-tdd-setup.yml', dest: '.github/workflows/auto-tdd-setup.yml' }, { src: 'github/workflows/tdd.yml', dest: '.github/workflows/tdd.yml' }, { src: 'github/ISSUE_TEMPLATE/feature_request.md', dest: '.github/ISSUE_TEMPLATE/feature_request.md' });
    }
    if (docs) {
        filesToCopy.push({ src: 'docs/TDD_WORKFLOW.md', dest: 'TDD_WORKFLOW.md' }, { src: 'docs/TESTING_COMPARISON.md', dest: 'TESTING_COMPARISON.md' }, { src: 'docs/VUE_TESTING_ALIGNMENT.md', dest: 'VUE_TESTING_ALIGNMENT.md' });
    }
    // Copy files
    let copiedCount = 0;
    let skippedCount = 0;
