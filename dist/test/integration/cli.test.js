...
describe('CLI Integration', () => {
    ...
    describe('init command', () => {
        ...
        it('should support --no-workflows flag', async () => {
            ...
            expect(initTDD).toHaveBeenCalledWith(expect.objectContaining({ workflows: false }));
        });
        it('should support --no-docs flag', async () => {
            const { initTDD } = await import('../../lib/init');
            await initTDD({ workflows: true, docs: false, force: false });
            expect(initTDD).toHaveBeenCalledWith(expect.objectContaining({ docs: false }));
        });
        it('should support --force flag', async () => {
            const { initTDD } = await import('../../lib/init');
            await initTDD({ workflows: true, docs: true, force: true });
            expect(initTDD).toHaveBeenCalledWith(expect.objectContaining({ force: true }));
        });
    });
    ...
    describe('feature command', () => {
        ...
        it('should support --no-issue flag', async () => {
            ...
            expect(createFeature).toHaveBeenCalledWith(expect.objectContaining({ issue: false }));
        });
    });
