...
describe('createFeature', () => {
    ...
    describe('successful creation', () => {
        it('should execute feature wizard script', async () => {
            ...
            await createFeature();
            expect(execSyncSpy).toHaveBeenCalledWith(expect.stringContaining('tdd-feature.js'), expect.objectContaining({
                cwd: mockCwd,
                stdio: 'inherit'
            }));
        });
        it('should execute script with default options', async () => {
            ...
            expect(execSyncSpy).toHaveBeenCalledWith(expect.any(String), expect.objectContaining({
                cwd: mockCwd,
                stdio: 'inherit'
            }));
        });
    ...
    describe('error handling', () => {
        it('should throw error if script execution fails', async () => {
            ...
            await expect(createFeature()).rejects.toThrow("Failed to start feature wizard. Make sure you've run 'vue-tdd init' first.");
        });
    ...
    describe('path handling', () => {
        ...
        it('should execute script in current working directory', async () => {
            const execSyncSpy = vi.mocked(execSync).mockImplementation(() => Buffer.from(''));
            await createFeature();
            expect(execSyncSpy).toHaveBeenCalledWith(expect.any(String), expect.objectContaining({ cwd: mockCwd }));
        });
        it('should use inherit stdio for interactive wizard', async () => {
            const execSyncSpy = vi.mocked(execSync).mockImplementation(() => Buffer.from(''));
            await createFeature();
            expect(execSyncSpy).toHaveBeenCalledWith(expect.any(String), expect.objectContaining({ stdio: 'inherit' }));
        });
    });
    ...
    describe('execution context', () => {
        it('should execute in the current working directory', async () => {
            ...
            const execSyncSpy = vi.mocked(execSync).mockImplementation(() => Buffer.from(''));
            await createFeature();
            expect(execSyncSpy).toHaveBeenCalledWith(expect.any(String), expect.objectContaining({ cwd: testCwd }));
        });
