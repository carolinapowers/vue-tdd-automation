...
describe('initTDD', () => {
    ...
    describe('validation', () => {
        ...
        it('should warn if Vue is not in dependencies', async () => {
            ...
            expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('Warning: Vue not found in dependencies'));
        });
    });
    describe('file copying', () => {
        ...
        it('should skip existing files when force is false', async () => {
            ...
            expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('Skipped (exists)'));
        });
        ...
        it('should create directories if they do not exist', async () => {
            ...
            expect(mkdirSyncSpy).toHaveBeenCalledWith(expect.any(String), expect.objectContaining({ recursive: true }));
        });
    });
    describe('package.json updates', () => {
        ...
        it('should not overwrite existing scripts', async () => {
            ...
            vi.mocked(fs.readFileSync).mockReturnValue(JSON.stringify(packageWithScripts));
            await initTDD();
        ...
        it('should preserve existing package.json properties', async () => {
            ...
            vi.mocked(fs.readFileSync).mockReturnValue(JSON.stringify(packageWithExtra));
            await initTDD();
            const writtenContent = writeFileSyncSpy.mock.calls[0]?.[1];
            const updatedPackageJson = JSON.parse(writtenContent);
    ...
    describe('console output', () => {
        ...
        it('should log installation progress', async () => {
            const consoleLogSpy = vi.spyOn(console, 'log');
            await initTDD();
            expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('Installing TDD infrastructure'));
        });
        ...
        it('should display required dependencies', async () => {
            const consoleLogSpy = vi.spyOn(console, 'log');
            await initTDD();
            expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('Required dependencies'));
            expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('npm install -D'));
        });
    });
});
